<!-- src/core/views/tasks/components/tasks-card.vue -->
<!-- Post-It Style Karte für Wiedervorlagen -->
<template>
    <v-card
        class="tasks-card mb-2"
        :class="[
            `bg-${color}`,
    ...
    >
        ...
        <v-card-text class="pa-3 pb-1">
            ...
            >
                {{ task.note?.subject || t('TasksCard.noSubject') }}
            </h4>
            ...
            <!-- Verknüpfung -->
            <div
                v-if="task.links && task.links.length > 0"
                class="mb-2"
            >
                <v-chip
        ...
        <v-card-actions
            v-if="task.assigned_employees?.length > 0"
            class="pt-0 px-3 pb-2"
        >
            <v-avatar
                v-for="(emp, idx) in task.assigned_employees.slice(0, 3)"
                :key="emp.id"
                size="24"
            ...
            <span
                v-if="task.assigned_employees.length > 3"
...
const formattedDate = computed(() => {
    const date = new Date(props.task.task_date);
    ...
    if (date.toDateString() === today.toDateString()) {
        return t('TasksCard.today');
    }
    if (date.toDateString() === tomorrow.toDateString()) {
        return t('TasksCard.tomorrow');
    }

    // Relative Zeit für nahe Daten
    const diffDays = Math.ceil((date - today) / (1000 * 60 * 60 * 24));
    if (diffDays < 0) {
        return t('TasksCard.daysAgo', { days: Math.abs(diffDays) });
    }
    if (diffDays <= 7) {
        return t('TasksCard.inDays', { days: diffDays });
    }
